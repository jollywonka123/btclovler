        ...
        if re.search(r' ', user_mess):
            ...
            if (genius + genius2) not in cheki:
                ...
                with open('db.txt', 'wb') as handle:
                    ...
                    await client.send_message(idchat, '[Clean]🧹 Чек был `очищен` от русских символов и пробелов: ' + genius + genius2)
        elif len(sex) >= 68:
            v = re.search(r'c_\nc_\S+', user_mess)
            ...
            if (geniuss) not in cheki:
                ...
                with open('db.txt', 'wb') as handle:
                    ...
                    await client.send_message('BTC_CHANGE_BOT', '/start ' + geniuss)
        elif len(sex) > 35:
            reg = re.compile('[^a-zA-Z0-9_]')
            genius = (reg.sub('', m.group(0)))
